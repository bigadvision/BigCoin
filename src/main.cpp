...
uint256 hashGenesisBlock("0x017daef39d9902b93e7f96b18bdea890cabf8399fade8dfc7f4e66a785cf7c61");
...
{
    // BigCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // BigCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "bigcoin genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0x65479cb18b451e8baaa57c3a9c0da200c74723f8ae79db8ce2993ce0f9fce2c3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd6, 0xfa, 0xaf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BigCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BigCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BigCoinMiner(CWallet *pwallet)
{
    printf("BigCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("BigCoin-miner");
    ...
    try { loop {
        ...
        printf("Running BigCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BigCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BigCoinMiner, pwallet));
}
